...
$stmt = $con->prepare("CALL jsp_uw_searchForums(?, ?, ?, ?)") or append_log_error("Prepare stm uw_searchForums failed: " .$con->error);
$stmt->bind_param('isii', $g_cust_id, $search_value, $start, $limit);

$stmt2 = $con->prepare("CALL jsp_uw_getForumReplies(?)") or append_log_error("Prepare stm uw_getForumReplies failed: " .$con->error);
$stmt2->bind_param('i', $frm_id);

$stmt->execute() or append_log_error('uw_searchForums exec: '.$stmt->error);
$stmt->store_result();
...
for ($i1=0; $i1<$n1; $i1++) {
  ...
  $frm_id = $row1[$i1][0];
  if($row1[$i1][8] > 0) {
    $stmt2->execute() or append_log_error('uw_getForumReplies exec: '.$stmt->error);
    $stmt2->store_result();
