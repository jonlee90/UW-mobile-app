...
if(isset($filter_cat)) {
  $stmt = $con->prepare("CALL jsp_uw_filterPhones(?, ?, ?, ?)") or append_log_error("Prepare stm phone-search.php uw_filterPhones failed: " .$con->error);
  $stmt->bind_param('iiss', $start, $limit, $search_value, $filter_cat);
}else {
  $stmt = $con->prepare("CALL jsp_uw_searchPhones(?, ?, ?)") or append_log_error("Prepare stm phone-search.php uw_searchPhones failed: " .$con->error);
  $stmt->bind_param('iis', $start, $limit, $search_value);
}
$stmt2 = $con->prepare("CALL jsp_uw_getPhoneAttr(?)") or append_log_error("Prepare stm phone-search.php uw_getPhoneAttr failed: " .$con->error);
$stmt2->bind_param('i', $phone_id);

$stmt3 = $con->prepare("CALL jsp_uw_getPhoneImg(?, ?)") or append_log_error("Prepare stm phone-search.php uw_getPhoneImg failed: " .$con->error);
$stmt3->bind_param('ii', $phone_id, $main_image_id);

$stmt->execute() or append_log_error('phone-search.php filter/search phones exec: '.$stmt->error);
$stmt->store_result();
...
for ($i1=0; $i1<$n1; $i1++) {
  ...
  $phone_id = $row1[$i1][0];
  $stmt2->execute() or append_log_error('phone-search.php uw_getPhoneAttr exec: '.$stmt->error);
  $stmt2->store_result();
  ...
  $stmt3->execute() or append_log_error('phone-search.php uw_getPhoneImg exec: '.$stmt->error);
  $stmt3->store_result();
